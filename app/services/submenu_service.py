...
def create_submenu_service(
    ...
    if not menu.is_active or menu.is_deleted:
        raise HTTPException(
            ...
            detail="Cannot add submenu to an inactive or deleted menu."
        )

    # Duplicate validation across DB
    duplicate_title = db.query(SubMenu).filter(
        SubMenu.menu_id == submenu.menu_id, 
        SubMenu.title == submenu.title
    ).first()
    if duplicate_title:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail="SubMenu title already exists in this menu."
        )
    
    if submenu.path:
        duplicate_path = db.query(SubMenu).filter(
            SubMenu.menu_id == submenu.menu_id, 
        ...
        if duplicate_path:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail="SubMenu path already exists in this menu."
            )
...
def update_submenu_service(
    ...
    if submenu.menu_id is not None:
        target_menu = get_menu_by_id(db, submenu.menu_id)
        if not target_menu or not target_menu.is_active or target_menu.is_deleted:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail=f"Target menu with id {submenu.menu_id} is invalid, inactive, or deleted."
            )

    # Validation on update
    check_menu_id = submenu.menu_id if submenu.menu_id is not None else db_submenu.menu_id
    if submenu.title:
        duplicate_title = db.query(SubMenu).filter(
            SubMenu.menu_id == check_menu_id, 
            SubMenu.title == submenu.title, 
