...
def create_menu(menu: MenuCreate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    return create_menu_service(db, menu)

#get menu_by_id
@router.get("/{menu_id}", response_model=MenuResponse)
def get_menu_by_id(menu_id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    return get_menu_by_id_service(db, menu_id)

#get all menu
@router.get("/", response_model=MenuListResponse)
def get_all_menus(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    return get_all_menu_service(db)

#update menu
@router.put("/{menu_id}", response_model=MenuResponse)
def update_menu(menu_id: int, menu: MenuUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
